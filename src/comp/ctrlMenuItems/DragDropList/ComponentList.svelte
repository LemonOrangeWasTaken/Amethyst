...
<div data-dnd-zone class={`${zoneClass} ${dropzone.containerClass}`} bind:this={dropzone.el}>
	{#each $collection as _, i (_.type)}
		...
		>
            <Element tagType={_.type} elmntIndex={i} height={layerHeight} width={containerWidth-20} iconURI={HTMltagInfo[_.type].iconURI}/>
            
            {#if !!_.styleOverrides}
                <section class="overrideContainer">
                    {#each _.styleOverrides as override}
                        <Override name={override.name} height={layerHeight} margin={10}/>
                    {/each}
                </section>
            {/if}
		</div>
